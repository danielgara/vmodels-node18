...
export class VariaMosGraph {
  ...
  public async loadModules(path:any) {
    ...
    try {
      const fetchFile = await fetch(`../src/assets/js/${path}`);
      if (fetchFile.headers.get('content-type') == 'application/javascript') {
        modelModule = await import(/* @vite-ignore */`../${path}`); // load custom modules
      }
    } catch (error) {
      // return null module
    }
  ...
  public async setCustomShapes() {
    ...
    try {
      ...
      const fetchFile = await fetch(`../src/assets/js/custom_models/${this.getCurrentModel().type}/xml/custom_shapes.xml`);
      if (fetchFile.headers.get('content-type') == 'application/xml') {
        const xmlSrc = await fetchFile.text();
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(xmlSrc, 'text/xml');
